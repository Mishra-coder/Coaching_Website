...
const ContestManager = () => {
    ...
    const removeQuestion = (index) => {
        if (currentContest.questions.length > 1) {
            const newQuestions = currentContest.questions.filter((_, i) => i !== index);
            setCurrentContest(prev => ({ ...prev, questions: newQuestions }));
        }
    };
    ...
    const getContestStatus = (contest) => {
        ...
        const end = new Date(contest.endTime || new Date(start.getTime() + contest.duration * 60000));
    ...
    return (
        <div className="admin-container">
            ...
            {showBulkUpload && (
                <div className="admin-card bulk-upload-card">
                    ...
                    <p className="bulk-upload-description">
                        Upload an Excel file (.xlsx, .xls) or CSV with contest questions. Maximum file size: 10MB
                    </p>
                    <div style={{ background: '#eff6ff', padding: '15px', borderRadius: '8px', marginBottom: '15px', border: '1px solid #bfdbfe' }}>
                        <p style={{ margin: '0 0 10px 0', color: '#1e40af', fontSize: '0.9rem', fontWeight: '600' }}>
                            <i className="fas fa-info-circle"></i> Required Excel Columns:
                        </p>
                        <ul style={{ margin: 0, paddingLeft: '20px', color: '#1e40af', fontSize: '0.85rem' }}>
                            <li><strong>Question Text</strong> - The question</li>
                            <li><strong>Option 1, Option 2, Option 3, Option 4</strong> - Four options</li>
                            <li><strong>Correct Answer</strong> - The correct option text</li>
                        </ul>
                        <p style={{ margin: '10px 0 0 0', color: '#1e40af', fontSize: '0.85rem' }}>
                            Optional: Add <strong>title</strong>, <strong>description</strong>, <strong>startTime</strong>, <strong>duration</strong> in first row
                        </p>
                    </div>
                    <div style={{ background: '#fef3c7', padding: '15px', borderRadius: '8px', marginBottom: '15px', border: '1px solid #fde68a' }}>
                        <p style={{ margin: 0, color: '#92400e', fontSize: '0.9rem' }}>
                            <i className="fas fa-star"></i> <strong>XP System:</strong> Each correct answer awards 3 XP. Students compete on the leaderboard!
                        </p>
                    </div>
                    <input 
                        type="file" 
            ...
            <form onSubmit={handleSubmit} className="admin-card question-form-card">
                <h3 className="question-form-title">{isEditing ? 'Edit Contest' : 'Add New Contest'}</h3>

                <div style={{ background: '#fef3c7', padding: '15px', borderRadius: '8px', marginBottom: '20px', border: '1px solid #fde68a' }}>
                    <p style={{ margin: 0, color: '#92400e', fontSize: '0.9rem' }}>
                        <i className="fas fa-star"></i> <strong>XP Reward:</strong> Each correct answer = 3 XP. Leaderboard ranks students by total XP earned!
                    </p>
                </div>
                ...
                <div className="question-form-grid">
                    ...
                    <h4 style={{ marginTop: '20px', marginBottom: '15px', color: '#1a237e' }}>Contest Questions</h4>
                    {currentContest.questions.map((q, qIndex) => (
                        <div key={qIndex} className="question-block" style={{ marginBottom: '20px', padding: '20px', border: '2px solid #e0e0e0', borderRadius: '12px', backgroundColor: '#f8fafc' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                                <h5 style={{ color: '#1a237e', fontSize: '1.1rem' }}>Question {qIndex + 1}</h5>
                    ...
                    {contests.map((contest) => (
                        <div key={contest._id} className="enrollment-card-mobile">
                            <div className="enrollment-card-header">
                                <div>
                                    ...
                                    <div className="enrollment-date">
                                        Start: {formatDateTime(contest.startTime)} • End: {formatDateTime(contest.endTime || new Date(new Date(contest.startTime).getTime() + contest.duration * 60000))} • {contest.questions.length} questions
                                    </div>
                            ...
                            <div className="question-card-actions">
                                ...
                                <button
                                    onClick={() => setDeleteDialog({ isOpen: true, id: contest._id })}
                                    className="btn-action btn-delete"
                                >
                                    Delete
                                </button>
                ...
                <div className="admin-table-wrapper enrollment-table-desktop">
                    <table className="admin-table">
                        ...
                        <tbody>
                            {contests.map((contest) => (
                                <tr key={contest._id}>
                                    <td className="question-table-text" style={{ fontWeight: '600' }}>{contest.title}</td>
                                    <td>{formatDateTime(contest.startTime)}</td>
                                    <td>{formatDateTime(contest.endTime || new Date(new Date(contest.startTime).getTime() + contest.duration * 60000))}</td>
            ...
            {showContestDetailsModal && (
                <div className="modal-overlay" onClick={() => {
                    ...
                    <div className="modal-content-box" onClick={e => e.stopPropagation()} style={{ maxWidth: '600px' }}>
                        <button onClick={() => {
                            setShowContestDetailsModal(false);
                            setBulkFile(null);
                        }} className="modal-close-btn">&times;</button>
                        
                        <h3 style={{ marginBottom: '20px', color: '#1a237e' }}>Contest Details</h3>
                        ...
                        <div className="form-group">
                            ...
                            <input
                                ...
                                onChange={(e) => setContestDetails({ ...contestDetails, title: e.target.value })}
                                placeholder="e.g., Weekly Maths Contest"
                                required
                                className="form-input"
                            />
                        ...
                        <div className="form-group">
                            ...
                            <input
                                ...
                                onChange={(e) => setContestDetails({ ...contestDetails, startTime: e.target.value })}
                                required
                                className="form-input"
                            />
                        </div>
                        ...
                        <div className="form-group">
                            ...
                            <input
                                ...
                                onChange={(e) => setContestDetails({ ...contestDetails, endTime: e.target.value })}
                                required
                                className="form-input"
                            />
                        </div>
                        ...
                        <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
                            <button 
                                ...
                                disabled={!contestDetails.title || !contestDetails.startTime || !contestDetails.endTime}
