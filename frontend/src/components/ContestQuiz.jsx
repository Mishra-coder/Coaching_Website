...
const ContestQuiz = () => {
  ...
  const loadContest = async () => {
    try {
      ...
      setAnswers(
        new Array(contestData.questions.length).fill({ selectedAnswer: '' })
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setToast({
        message: err.response?.data?.message || 'Failed to submit contest',
        type: 'error',
      });
      setSubmitting(false);
    }
      ...
      <div className="quiz-card">
        <div className="quiz-header">
          ...
          <div
            ...
            style={{ color: timeLeft < 60 ? '#ef4444' : '#1a237e' }}
          >
        ...
        <div className="quiz-navigation">
          ...
          {currentQuestion === contest.questions.length - 1 ? (
            ...
            <button
              onClick={() =>
                setCurrentQuestion(
                  Math.min(contest.questions.length - 1, currentQuestion + 1)
                )
              }
              className="btn-primary"
            >
