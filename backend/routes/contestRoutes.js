...
router.post('/bulk-upload', protect, authorize('admin'), async (req, res) => {
    try {
        const { fileData, title: providedTitle, description: providedDescription, startTime: providedStartTime, endTime: providedEndTime, duration: providedDuration } = req.body;
        ...
        if (fileData.includes('text/csv') || fileData.includes('spreadsheet')) {
            ...
            let endTime = providedEndTime || new Date(new Date(startTime).getTime() + (providedDuration || 60) * 60000).toISOString();
            ...
            for (let i = 0; i < jsonData.length; i++) {
                ...
                let correctAnswer = row['Correct Answer'] || row.correctAnswer || row.CorrectAnswer || row.correct_answer;
                ...
                if (questionText && option1 && option2 && option3 && option4 && correctAnswer) {
                    ...
                    if (correctAnswer.toLowerCase() === 'option 1' || correctAnswer === '1') {
                        finalCorrectAnswer = options[0];
                    } else if (correctAnswer.toLowerCase() === 'option 2' || correctAnswer === '2') {
                        finalCorrectAnswer = options[1];
                    } else if (correctAnswer.toLowerCase() === 'option 3' || correctAnswer === '3') {
                        finalCorrectAnswer = options[2];
                    } else if (correctAnswer.toLowerCase() === 'option 4' || correctAnswer === '4') {
            ...
            if (questions.length === 0) {
                return res.status(400).json({ 
                    success: false, 
                    message: 'No valid questions found in file. Please ensure columns are named: "Question Text", "Option 1", "Option 2", "Option 3", "Option 4", "Correct Answer"' 
                });
            }
        ...
        res.status(500).json({ 
            success: false, 
            message: err.message || 'Failed to upload contest' 
        });
    }
});
...
router.get('/:id/my-result', protect, async (req, res) => {
    try {
        ...
        const result = await ContestResult.findOne({ contest: contestId, user: userId })
