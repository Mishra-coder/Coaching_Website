  ...
  try {
    ...
    if (fileData.includes('text/csv') || fileData.includes('spreadsheet')) {
      ...
      let endTime =
        providedEndTime ||
        new Date(
          new Date(startTime).getTime() + (providedDuration || 60) * 60000
        ).toISOString();
      ...
      for (let i = 0; i < jsonData.length; i++) {
        ...
        const questionText =
          row['Question Text'] || row.question || row.Question;
      ...
      if (questions.length === 0) {
        return res.status(400).json({
          success: false,
          message:
            'No valid questions found in file. Please ensure columns are named: "Question Text", "Option 1", "Option 2", "Option 3", "Option 4", "Correct Answer"',
        });
      }
